...
# 独立Logger
wallpaper_logger = logging.getLogger("octopus_wallpaper")
wallpaper_logger.setLevel(logging.INFO)
wallpaper_logger.propagate = False
...
wallpaper_logger.addHandler(file_handler)
...
def update_config(**kwargs):
    ...
    if os.path.exists(config_path):
        try:
            ...
            wallpaper_logger.warning(f"读取配置文件失败，将重建新配置: {e}")
            config = {}   # 文件损坏，重新开始
    ...
    try:
        ...
        wallpaper_logger.info(f"配置文件已更新: {config_path}，{config}，{f}")
    except PermissionError:
        wallpaper_logger.error("更新配置文件失败：没有写入权限！请以管理员身份运行程序")
    except Exception as e:
        wallpaper_logger.error(f"更新配置文件失败：{e}")
...
def load_wallpaper_path() -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        if not wallpaper_path or not os.path.isfile(wallpaper_path):
            if wallpaper_path:
                wallpaper_logger.warning(f"配置文件中的路径无效（文件不存在）：{wallpaper_path}")
            else:
                wallpaper_logger.info("配置文件中无有效壁纸路径")
            return None, None

        # 最终校验通过
        wallpaper_logger.info(f"从配置文件加载成功：路径={wallpaper_path}，类型={wallpaper_type}")
        return wallpaper_path, wallpaper_type

    except json.JSONDecodeError:
        wallpaper_logger.error(f"配置文件格式错误（JSON解析失败）：{config_path}，重建配置文件")
        init_config_file()  # 重建配置
        return None, None  # 重建后无有效路径，返回None

    except PermissionError:
        wallpaper_logger.error(f"无权限读取配置文件：{config_path}（请以管理员身份运行）")
        return None, None

    except FileNotFoundError:
        wallpaper_logger.error(f"配置文件不存在（重建失败）：{config_path}")
        return None, None

    except Exception as e:
        wallpaper_logger.error(f"读取配置文件未知错误：\n\t {e} \n\t（路径={config_path}）")
        return None, None
...
def is_autostart_enabled():
    ...
    wallpaper_logger.debug(f"开机自启状态检查: {path} 是否存在 = {enabled}")
    return enabled
    ...
    try:
        if enable:
            if not os.path.exists(shortcut_path):
                ...
                shortcut.save()
                wallpaper_logger.info(f"开机自启已启用，快捷方式创建于: {shortcut_path}")
            else:
                wallpaper_logger.debug("开机自启快捷方式已存在，无需重复创建")
        else:
            if os.path.exists(shortcut_path):
                os.remove(shortcut_path)
                wallpaper_logger.info(f"开机自启已禁用，快捷方式已删除: {shortcut_path}")
            else:
                wallpaper_logger.debug("开机自启快捷方式不存在，无需删除")
    except Exception as e:
        wallpaper_logger.exception(f"设置开机自启失败 (enable={enable})")
        raise
...
def check_NOT_USE_WX(file_path):
    ...
    try:
        ...
        wallpaper_logger.warning(f"检查 NOT_USE_WX 时出错: {e}")
    return False